...
func NewUiQuadPass(device *wgpu.Device, surfaceFormat wgpu.TextureFormat) (*Pass, error) {
...
func uiQuadPrepare(s any, context *PassContext) error {
...
func uiQuadExecute(s any, context *PassContext) error {
	state := s.(*uiQuadPassState)
	if state.count == 0 {
		return nil
	}
